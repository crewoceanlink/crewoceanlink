...
function CrewLoginContent() {
  ...
  const loginWithVoucher = async (code) => {
    setLoading(true);
    setError("");
    ...
    try {
      ...
      window.localStorage.setItem("crew_voucher_code", cleanCode);

      router.push(
        `/crew/dashboard?voucher=${encodeURIComponent(cleanCode)}&session=${encodeURIComponent(
          data.session_id
        )}`
      );
    } catch (err) {
  ...
  return (
    <div className="h-screen w-full relative text-white overflow-hidden">
      <img src="/msc-ship.jpg" className="absolute w-full h-full object-cover" alt="msc ship" />
      ...
      <div className="absolute top-6 left-7 right-7 flex justify-between items-center z-20">
        ...
        <span className="text-white text-xl font-medium">Crew Login</span>
      </div>

      <div className="relative z-10 flex items-end justify-center min-h-screen pb-4 px-4 pt-20">
        <div className="w-full h-[calc(100vh-6rem)] mx-auto rounded-3xl bg-white/[0.02] backdrop-saturate-150 backdrop-blur-[4px] border border-white/20 shadow-[0_8px_32px_rgba(0,0,0,0.25)] p-4 flex flex-col">
          ...
          <div className="mt-6 rounded-2xl bg-white/5 backdrop-blur-[6px] border border-white/15 shadow-inner p-4">
            <form onSubmit={handleLogin} className="space-y-4">
              <input
                ...
                autoCorrect="off"
                spellCheck="false"
                className="w-full rounded-2xl bg-white/15 text-white placeholder:text-white/60 px-4 py-4 border border-white/20 outline-none text-base"
              />

              <button
                type="submit"
                disabled={loading}
                className="w-full rounded-2xl bg-white px-4 py-4 border border-white/20 shadow-[0_4px_20px_rgba(0,0,0,0.25)] text-gray-700 font-semibold text-lg disabled:opacity-60"
              >
